# Guide Utilisateur Scolaris 📖

Bienvenue sur Scolaris. Ce guide vous aide à utiliser les fonctionnalités selon votre rôle.

## 🎖️ Super Administrateur (Global)
- **Gestion des Écoles** : Seul le super_admin peut créer de nouveaux établissements scolaires dans le système.
- **Supervision** : Vue d'ensemble sur toutes les écoles partenaires de la plateforme.

## 👑 Administrateur d'École (Directeur)
- **Gestion du Personnel** : Création et attribution des comptes enseignants au sein de son école.
- **Paramétrage** : Configuration des classes et des informations de l'établissement.
- **Supervision Locale** : Suivi des statistiques de réussite de son école.

## 👨‍🏫 Enseignant / Directeur
- **Gestion des Élèves** : Inscrire des élèves et générer leurs matricules.
- **Bulletins de Notes** : Saisir les notes par matière et par trimestre.
- **Devoirs** : Publier des leçons et des exercices (possibilité de joindre des PDF/Images).
- **Communications** : Envoyer des communiqués et des convocations officielles.
- **Contact Parent** : Boutons rapides pour contacter les parents via WhatsApp ou SMS.

## 👪 Parent
- **Tableau de Bord** : Suivre les devoirs et les notes de tous ses enfants.
- **Suivi des Notes** : Visualiser le tableau récapitulatif annuel.
- **Bulletin Numérique** : Accéder au portail interactif avec barres de progression.
- **Signature Numérique** : Valider les devoirs de son enfant via un code de sécurité SMS.
- **Communications** : Recevoir et lire les annonces de l'école.

## ✍️ Comment signer un devoir ?
1. Cliquez sur le devoir dans votre espace.
2. Saisissez votre nom complet.
3. Demandez le code de sécurité.
4. Saisissez le code à 6 chiffres reçu par SMS (simulation dans le terminal).
5. Confirmez pour valider.
